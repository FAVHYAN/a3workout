...
class Checkout extends Front_Controller {
	...
	*/
	...
	{		
		...
		$customer 	= $this->go_cart->customer();
		$cart 		= $this->go_cart->_cart_contents;
		foreach ($cart['items'] as $item) {
				$this->db->insert("student_packages", array("id_customer" => $customer['id'], "id_package" => $item['id'], "date" => date("Y-m-d")));
		}
