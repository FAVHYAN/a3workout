		...
		<thead>
			<thead>
				<tr>
					...
					<th style="width:11%;"><?php echo lang('quantity');?></th>
						...
						<?php if($this->uri->segment(1) == 'cart'): ?>
							<?php if(!(bool)$product['fixed_quantity']):?>
							    <div class="input-group">
							      <input type="text" name="cartkey[<?php echo $cartkey;?>]" value="<?php echo $product['quantity'] ?>" size="3" class="form-control">
							      <span class="input-group-btn">
							        <button class="btn btn-danger" type="button" onclick="if(confirm('<?php echo lang('remove_item');?>')){window.location='<?php echo site_url('cart/remove_item/'.$cartkey);?>';}"><i class="glyphicon glyphicon-remove"></i></button>
							      </span>
							    </div>
							<?php else:?>
