...
<script  type="text/javascript">
    ...
    function verHorario(horario, intervalo, verHorario, fecha){
        ...
        while(compararHora(hora.getHours()+":"+hora.getMinutes(), auxHorario[1])){            
            ...
            var date = (fecha.getYear()+1900)+"_"+Mes+"_"+fecha.getDate()+"_"+hours+"_"+Minutes+"_00";   
                ...
                success:  function (response) { 
                    ...
                    $.each(obj, function() {                                                   
                        ...
                        if(this['type'] == 1){  
                            ...
                            var note  = this['description'];
                            if(note == null){
                                note = 'don´t have note';
                            }else{
                            ...
                            $.each(obj_photo, function() { 
                                ...
                                if(induction > 0){                                    
                                   $("#"+fecha).html("<div class='busy' style='background-color:#57A349' onclick=\"popover_profile_student_class_induction('"+fecha+"','"+photo+"','"+note+"','"+name+"','"+username+"','"+id_class+"','"+date+"','"+id_trainer+"')\" title='"+firstname+" "+lastname+"'>"+firstname+" "+lastname+"</div>"); 
                                } else{
                                    $("#"+fecha).html("<div class='busy'  onclick=\"popover_profile_student('"+fecha+"','"+photo+"','"+note+"','"+name+"','"+username+"','"+id_class+"','"+date+"','"+id_trainer+"')\" title='"+firstname+" "+lastname+"'>"+firstname+" "+lastname+"</div>"); 
                                }
                            });
                        }else if(this['type'] == 3){  
                            ...
                            $.each(obj_photo, function() { 
                                var photo = this["filename"];
                                if(induction > 0){                                    
                                   $("#"+fecha).html("<div class='busy' style='background-color:#0a374a' onclick=\"popover_profile_student_class_induction('"+fecha+"','"+photo+"','"+note+"','"+name+"','"+username+"','"+id_class+"','"+date+"','"+id_trainer+"')\" title='"+firstname+" "+lastname+"'>"+firstname+" "+lastname+"</div>"); 
                                } else{
                                    $("#"+fecha).html("<div class='busy'  onclick=\"popover_profile_student('"+fecha+"','"+photo+"','"+note+"','"+name+"','"+username+"','"+id_class+"','"+date+"','"+id_trainer+"')\" title='"+firstname+" "+lastname+"'>"+firstname+" "+lastname+"</div>"); 
                                }
                            });
                   ...
                   $.each(obj, function() { 
                       ...
                       var obj_photo = $.parseJSON(this['photo']);   
                           $.each(obj_photo, function() { 
                               ...
                               contenido+= '<div class="row">'+ 
                                               ...
                                               '</div>'+
                                               "<div class='col-md-9 text_justify'><a  onclick=\"location.href='http://www.a3workout.com/index.php/"+username+"'\"><b>"+name+"</b></a><br />Trainer´s note"+note.substr(0,100)+"...</div>"+
                                           '</div><br />';
                       });
                   });
                    ...
                    contenido+=     '<div class="row" style="margin-top:10px">'+ 
                                        '<div class="col-md-12" style="text-align:center">'+
                                             ...
                                             "<div style='float: left;'><button style='margin: 0px 20px 0px 20px;' onclick=\"go_to_class_group('"+id_trainer+"','"+id_student+"','"+name_campo+ "','"+name_trainer+"');\" class='btn btn-primary' style='font-size: 11px;'>Go to class</button></div>"+
                                             "<div style='float: right;height: 36px;margin-top: -36px;'><a data-toggle='modal' data-target='#modal_details' data-id="+name_campo+"><button style='height: 36px;' class='btn btn-primary' style='font-size: 11px;'>Class details</button></a></div>"+
                                        '</div>'+
                                    '</div>';
                    contenido+= '</div>';
                    btn2.popover({
    ...
    function popover_profile_student(name_campo, photo, note, name, username, id_class, date, id_trainer){
	    var id_trainer = $("#id_trainer").val();
        var id_student = $("#customer_id").val();
		var name_trainer = $("#name_trainer").val();
        ...
        var contenido = '<div id="popOverBox" style="font-size:14px;font-weight:bold">';
            contenido+= '<div class="row">'+ 
                            ...
                            '</div>'+
                            "<div class='col-md-7 text_justify'><a  onclick=\"location.href='http://www.a3workout.com/index.php/"+username+"'\"><b>"+name+"</b></a><br />Trainer´s note<br />"+note.substr(0,120)+"...</div>"+
                        '</div>'+
...
<form id="form2" action="../../../videoconference/videowhisper_conference.php" method="post" name="adminForm">
	...
	<input id="usertype" type="hidden" maxlength="32" size="16" value="3" name="usertype"> <!--administrador de al video conferencia-->
                    ...
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                     <h4 class="modal-title">CLASS DETAILS</h4>
                                            ...
                                            <br />
                                                <div id="details">
                     <?php 
                                        $query1 = $this->db->query("SELECT count(scheduler_events.date) as contador,scheduler_events.*,customers.id,customers.firstname,customers.lastname,customers.photo,customers.birthday,
                                            DATE_FORMAT(scheduler_events.date, '%Y_%c_%e_%H_%i_%s') AS fecha FROM scheduler_events
                                            INNER JOIN customers ON customers.id = scheduler_events.id_trainer
                                            WHERE scheduler_events.id_trainer ='".$user_logged->id."' AND scheduler_events.id_student is not NULL");
                                      ...
                                      foreach ($query1->result() as $key) {
                                         ...
                                         $contador = $key->contador;
                                          ...
                                          echo '<div><span class="text-danger">'.$contador.'</span> Students enrolled in this class.</div><br>';
                                          
                                         if($contador > 0){

                                            $query1 = $this->db->query("SELECT scheduler_events.*,customers.*,DATE_FORMAT(scheduler_events.date, '%Y_%c_%e_%H_%i_%s') AS fecha FROM scheduler_events INNER JOIN customers ON customers.id = scheduler_events.id_student WHERE scheduler_events.id_trainer ='".$user_logged->id."' AND scheduler_events.id_student is not NULL");
                                      ...
                                      foreach ($query1->result() as $key) {
                                        ?>
                                         ...
                                         <?php 
                                            ...
                                            ?>
                                            <?php echo 'There are no students enrolled in this class.';?>
