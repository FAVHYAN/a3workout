...
function add_course_image(data)
{
	p	= data.split('.');
...
var valor= $("#type").val();
if (valor==1){
$('#datefe').prop("disabled","disabled");
}else{
$('#datefe').prop("disabled","");
}
});
...
<?php echo form_open($this->config->item('admin_folder').'/courses/form/'.$id ); ?>
	...
	<div class="span12">
		...
		<div class="tab-content">
			<div class="tab-pane active" id="product_info">
                 ...
                 <div class="row">
					<div class="span8">
						 ...
						 <?php
                                                    ...
                                                    echo form_dropdown('enabled', $options, set_value('enabled',$enabled), 'class="span"');
                                                ?>
					</div>
				</div>
				 ...
				 <div class="row">
					<div class="span8">
						                 <label>One by one or Group</label>
					...
					if(empty($id)){
						...
						?>
						 <option>Select</option>
                                        <option value="1" name="1">One by one</option>
                                        <option value="2" name="2">Group</option>
						<?php

					}else{
						$query = $this->db->query("SELECT * FROM courses where courses.id = '".$id."'");
				...
				<div class="row">
					<div class="span8">
						...
						<?php
						$data	= array('name'=>'trainer', 'class'=>'span8');
                                                $options = array();

                                                $result=$this->db->get_where('customers', array('band'=>"2"));
                                                ...
                                                foreach ($result->result() as $row) {
                                                    ...
                                                    $options[$row->id] = $row->firstname;                                                    
                                                }
                                                
                                                echo form_dropdown('trainer', $options, $trainer, $data); 
						?>
					</div>
				</div>
                       ...
                       <label for="date">Date</label>
						...
						foreach ($query->result() as $key) {
							...
							if($key->type == 2){
								...
								list($date, $hour) = explode(' ', $key->data);?>
							
							<input class="datepicker" type='text' class="form-control" name='date'  id='datefe'  value="<?php echo $date;?>"/>
							<input id="hour"  type='text' class="form-control"  name="hour" value="<?php echo $hour;?>">
					...
					<div class="span2" style="text-align:center">
						<div class="row">
							<div class="span2">
								...
								<select class="span2" id="category_search" />
								...
								{
									$.post("<?php echo site_url($this->config->item('admin_folder').'/categories/category_autocomplete_courses_products/');?>", { name: $('#category_search').val(), limit:10, type : 1},
										function(data) {
			...
			<div class="tab-pane" id="product_photos">
				...
				<div class="row">
					<div class="span8">
                        ...
                        <input type="hidden" value="<?php echo $id?>" name="id_course" id="id_course">
                        <?php if($id == "viewschedule"){ ?>
