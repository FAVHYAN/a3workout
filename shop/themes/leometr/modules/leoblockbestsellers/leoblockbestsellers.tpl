...
<div id="best-sellers_block_right" class="block products_block">
	<h4 class="title_block"><a href="{$link->getPageLink('best-sales')}">{l s='Top sellers' mod='leoblockbestsellers'}</a></h4>
	...
	{if $best_sellers|@count > 0}
		<ul class="product_images">
			...
			<li class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} clearfix">
				<a href="{$product.link}" title="{$product.legend|escape:'htmlall':'UTF-8'}" class="content_img clearfix">
					...
					<img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'small_leometr')}" height="{$smallSize.height}" width="{$smallSize.width}" alt="{$product.legend|escape:'htmlall':'UTF-8'}" />
		...
		<p class="lnk"><a href="{$link->getPageLink('best-sales')}" title="{l s='All best sellers' mod='leoblockbestsellers'}" class="button_large">&raquo; {l s='All best sellers' mod='leoblockbestsellers'}</a></p>
	{else}
