...
<div class="products_block">
	<table id="product_comparison">
		...
		{foreach from=$products item=product name=for_products}
			...
			<td width="{$width}%" class="ajax_block_product comparison_infos"><div class="product-container">
				<a href="{$product->getLink()}" title="{$product->name|escape:html:'UTF-8'}" class="product_image" >
					<img src="{$link->getImageLink($product->link_rewrite, $product->id_image, 'home_leometr')}" alt="{$product->name|escape:html:'UTF-8'}" width="{$homeSize.width}" height="{$homeSize.height}" />
				</a>
