...
<div id="product_list" class="products_block view-grid"><div class="rows-fluid">
	{foreach from=$products item=product name=products}
		...
		<div class="p-item myspan{(12/Configuration::get('productlistcols'))} product_block ajax_block_product {if $smarty.foreach.products.first}first_item{elseif $smarty.foreach.products.last}last_item{/if} {if $smarty.foreach.products.index % 2}alternate_item{else}item{/if} clearfix">
			<div class="list-products">
					<div class="product-container clearfix">						
							<div class="center_block">
								<a href="{$product.link|escape:'htmlall':'UTF-8'}" class="product_img_link" title="{$product.name|escape:'htmlall':'UTF-8'}">
									<img src="{$link->getImageLink($product.link_rewrite, $product.id_image, 'home_leometr')}" alt="{$product.legend|escape:'htmlall':'UTF-8'}" {if isset($homeSize)} width="{$homeSize.width}" height="{$homeSize.height}"{/if} />
							...
							<div class="right_block">
								...
								<div class="product_desc"><a href="{$product.link|escape:'htmlall':'UTF-8'}" title="{$product.description_short|strip_tags:'UTF-8'|truncate:360:'...'}" >{$product.description_short|strip_tags:'UTF-8'|truncate:60:'...'}</a></div>
								...
								<span class="online_only">{if isset($product.online_only) && $product.online_only}{l s='Online only!'}{/if}</span>
								{/if}
