		...
		$this->_html .= '			
				...
				foreach ($this->_languages as $language)
					...
					<div id="cdescription_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
						<textarea id="description_'.$language['id_lang'].'" name="description_'.$language['id_lang'].'" cols="23" row="3">'.htmlentities(LeoBtmegamenuHelper::getFieldValue($obj, 'description', (int)($language['id_lang'])), ENT_COMPAT, 'UTF-8').'</textarea><sup> *</sup>
					</div>';
		...
		$this->_html .= $this->displayFlags($this->_languages, $this->_defaultFormLanguage, $divLang, 'ctype_html', true);
			$this->_html .= '
				...
				<div class="margin-form">
					...
					'.($obj->image ? '<br/><img src="'._MODULE_DIR_.$this->name.'/icons/'.$obj->image.'" alt="'.$this->l('icon').'">' : '').'
				</div>
