...
?>
	...
	<?php if(count($blocks) < LOF_MODULE_ADVANCE_CUSTOM_LIMIT_BLOCK){ ?>
		<a onclick="return false;" class="lofaddnew-block-item lofaddnew-block-<?php echo $id_position;?>" href="<?php echo _MODULE_DIR_.$this->name.'/popup.php?id_shop='.(int)($this->context->shop->id).'&id_position='.$id_position.'&addBlock&bo_theme='.$employee->bo_theme.'&id_divposition=lofposition-'.$id_position.'&id_lang='.$cookie->id_lang.'&nb='.count($blocks).'&token='.Tools::getValue('token').'&secure_key='.$this->secure_key;?>" title="add"><img border="0" src="../img/admin/add.gif"/><?php echo $this->l('Add new block');?></a><br/>
	<?php
	}
	if($blocks){
		foreach($blocks as $key=>$b){
		...
		?>
			<div class="lof-block <?php echo ($key%3 == 2 ? 'lof-block-right' : '');?>">
				...
				<a onclick="return false;" class="lofaddnew-block-item" href="<?php echo _MODULE_DIR_.$this->name.'/popup.php?id_shop='.(int)($this->context->shop->id).'&id_loffc_block='.$b['id_loffc_block'].'&addItem&bo_theme='.$employee->bo_theme.'&id_loftable=loftable-'.$b['id_loffc_block'].'&id_lang='.$cookie->id_lang.'&token='.Tools::getValue('token').'&secure_key='.$this->secure_key;?>" title="add"><img border="0" src="../img/admin/add.gif"/><?php echo $this->l('Add new item');?></a><br/>
				<table class="table loftable tableDnD" id="loftable-<?php echo $b['id_loffc_block'];?>">
					...
					?>
						<tr id="tr_<?php echo $b['id_loffc_block'];?>_<?php echo $item['id_loffc_block_item'];?>_<?php echo $item['position'];?>" class="tr_0_<?php echo $item['id_loffc_block_item'];?>_<?php echo $b['id_loffc_block'];?> <?php echo ($key%2 ? '' : 'alt_row');?>">
							...
							<td>
							<a class="display lofaddnew-block-item" href="<?php echo _MODULE_DIR_.$this->name.'/popup.php?id_shop='.(int)($this->context->shop->id).'&id_loffc_block='.$b['id_loffc_block'].'&addItem&bo_theme='.$employee->bo_theme.'&id_loffc_block_item='.$item['id_loffc_block_item'].'&id_loftr=tr_0_'.$item['id_loffc_block_item'].'_'.$b['id_loffc_block'].'&id_lang='.$cookie->id_lang.'&token='.Tools::getValue('token').'&secure_key='.$this->secure_key; ?>" title="edit">
