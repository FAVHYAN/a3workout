					...
					<script type="text/javascript">
						jQuery(document).ready(function(){
							...
							<tr id="tr_<?php echo $objItem->id_loffc_block;?>_<?php echo $objItem->id;?>_<?php echo $objItem->position;?>" class="tr_0_<?php echo $objItem->id;?>_<?php echo $objItem->id_loffc_block;?> <?php echo (count($items)%2 ? 'alt_row' : '');?>">\
								...
								<td><a class="display lofaddnew-block-item" href="<?php echo _MODULE_DIR_.$module->name.'/popup.php?id_loffc_block='.$objItem->id_loffc_block.'&addItem&bo_theme='.Tools::getValue('bo_theme').'&id_loffc_block_item='.$objItem->id.'&id_loftr=tr_0_'.$objItem->id.'_'.$objItem->id_loffc_block.'&id_lang='.Tools::getValue('id_lang',$cookie->id_lang).'&token='.Tools::getValue('token').'&secure_key='.$module->secure_key;; ?>" title="edit">\
