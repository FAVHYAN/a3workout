...
class LofTwitter extends Module
{
    ...
    */
	...
	{ 
		...
		if(_PS_VERSION_ <= "1.4"){
			...
			if( $theme && $theme != -1 ){
				if ( $showMode == "ticker"){
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/style.css", 'all');
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/li_scroller.css", 'all');
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/black.css", 'all');
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/blue.css", 'all');
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/white.css", 'all');
				}elseif ( $showMode == "block" ){
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/block.css", 'all');
				}else{
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/style.css", 'all');
					$this->context->controller->addCSS(_MODULE_DIR_.$this->name."/tmpl/".$theme."/assets/css/jScrollPane15.css", 'all');
				}
				
			}
			if ( $showMode == "ticker"){
				$this->context->controller->addJS(_MODULE_DIR_.$this->name."/assets/jquery.li-scroller.1.0.js");
			}
			elseif ( $showMode == "block" ){
				$this->context->controller->addJS(_MODULE_DIR_.$this->name."/assets/jcarousellite.js");
			}
			else{
				$this->context->controller->addJS(_MODULE_DIR_.$this->name."/assets/jScrollPane15/jquery.mousewheel.js");
				$this->context->controller->addJS(_MODULE_DIR_.$this->name."/assets/jScrollPane15/jquery.jscrollpane.min.js");
				$this->context->controller->addJS(_MODULE_DIR_.$this->name."/assets/jScrollPane15/mwheelIntent.js");
			}
			$this->context->controller->addJS(_MODULE_DIR_.$this->name."/assets/jtweetsanywhere.js");
    ...
    */
	function processHook($pparams, $pos="home"){
		...
		// get params
		$tmp 			= $params->get( 'module_height', '130' );
		...
		$title 			= $params->get( 'title' , 'Lof Twitter');
		$theme 			= $params->get( 'module_theme' , 'default');
		$showMode      = $params->get( 'showMode' , 'ticker');
		...
		$username 		= (count($username) > 1 ? "['".implode("','",$username)."']" : "'".implode("','",$username)."'");
