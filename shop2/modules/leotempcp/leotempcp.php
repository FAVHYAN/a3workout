...
class Leotempcp extends Module
{
	...
	public function install()
	{
		if (!parent::install()
			...
			return false;
	    $this->installModuleTab('LeoTheme Control Panel', 'panel', 'AdminParentModules');
		$this->_installConfig();
		return true;
	}
	...
	private function _displayForm()
	{
		...
		<form method="post" action="'.$this->base_config_url.'" enctype="multipart/form-data">
			<fieldset>
				...
				<li>+ '. $this->l( "Configuration For <b>" . _THEME_NAME_ . "</b> Theme " ) .'</li>
				</ul>
	...
	function hookHeader(){
		...
		if( $this->themeInfo ){
			...
			if( $paneltool ){
				...
				$this->context->controller->addJS(  __PS_BASE_URI__.'themes/'._THEME_NAME_."/info/assets/form.js" );
				$this->context->controller->addCss(  __PS_BASE_URI__.'themes/'._THEME_NAME_."/info/assets/form.css" );
			}
			...
			if( file_exists(_PS_ALL_THEMES_DIR_. _THEME_NAME_."/css/bootstrap.css") ){
				$bootstrapCss = __PS_BASE_URI__.'themes/'._THEME_NAME_."/css/bootstrap.css";
			}
			if( file_exists(_PS_ALL_THEMES_DIR_. _THEME_NAME_."/css/bootstrap-responsive.css") ){
				$bootstrapResponsive = __PS_BASE_URI__.'themes/'._THEME_NAME_."/css/bootstrap-responsive.css";
			}
			
			//
			$sql = 'SELECT *
				FROM `'._DB_PREFIX_.'leohook` WHERE theme="'._THEME_NAME_.'" AND id_shop='.(int)($this->context->shop->id);	
			...
			if( is_dir(_PS_ALL_THEMES_DIR_. _THEME_NAME_."/layout/".trim($layout)) &&  $layout ){
